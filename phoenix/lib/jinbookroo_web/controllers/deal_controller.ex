defmodule JinbookrooWeb.DealController do
  ...
  def create(conn, deal_params) do
    with {:ok, %Deal{} = deal} <- Books.create_deal_with_entries(deal_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id} = deal_params) do
    deal = Books.get_deal!(id) |> Books.with_entries()

    with {:ok, %Deal{} = deal} <- Books.update_deal_with_entries(deal, deal_params) do
      render(conn, "show.json", deal: deal |> Books.with_entries())
    end
  end
